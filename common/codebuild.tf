...
module "terraform_package_no_tagging" {
  source             = "../modules/codebuild-project"
  name               = "${local.common_name}-terraform-package-no-tag"
  description        = "${local.common_name}-terraform-package-no-tag"
  build_timeout      = "30"
  service_role       = aws_iam_role.codebuild.arn
  tags               = local.tags
  log_group          = module.create_loggroup.loggroup_name
  buildspec          = templatefile("./templates/terraform/package-no-tagging.yml.tpl", {})
  build_compute_type = "BUILD_GENERAL1_SMALL"
  build_type         = "LINUX_CONTAINER"
  environment = {
    compute_type                = "BUILD_GENERAL1_SMALL"
...
module "terraform_version_ssm" {
  source             = "../modules/codebuild-project"
  name               = "${local.common_name}-terraform-version-ssm"
  description        = "${local.common_name}-terraform-version-ssm"
  build_timeout      = "30"
  service_role       = aws_iam_role.codebuild.arn
  tags               = local.tags
  log_group          = module.create_loggroup.loggroup_name
  buildspec          = templatefile("./templates/terraform/version-ssm.yml.tpl", {})
  build_compute_type = "BUILD_GENERAL1_SMALL"
  build_type         = "LINUX_CONTAINER"
  environment = {
    compute_type                = "BUILD_GENERAL1_SMALL"
