resource "aws_codebuild_project" "oracle_validate_backup_codebuild_project" {
  
  for_each = { for job in var.oracle_validate_backup_jobs: "${job.environment}-${job.host}" => job }

    name           = "oracle-validate-backup-${each.key}"
    description    = "Validate backup for ${each.value.host} on ${each.value.environment}"
    ...
    logs_config {
      cloudwatch_logs {
        ...
        stream_name = "oracle-validate-backup-${each.key}"
      }
    }
